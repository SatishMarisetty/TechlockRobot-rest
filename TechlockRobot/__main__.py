...
I CAN ALSO PLAY SONGS ON EITHER GROUP OR CHANNEL USE `/VCHELP` FOR MUSIC PLAYER COMMANDS. """

buttons = [
    [
        InlineKeyboardButton( text="➕ Aᴅᴅ Mᴇ Tᴏ Yᴏᴜʀ Gʀᴏᴜᴘ ➕", url="t.me/techlock_bot?startgroup=true"),
    ],
    [
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("TechlockRobot.modules." + module_name)
...
@run_async
def erenn_data_callback(update, context):
    query = update.callback_query
    first_name = update.effective_user.first_name
    if query.data == "erenn_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="🔙", callback_data="erenn_back")
                 ]
                ]
            ),
        )
    elif query.data == "erenn_back":
        query.message.edit_text(
                PM_START_TEXT.format(
                    escape_markdown(first_name), ),
                reply_markup=InlineKeyboardMarkup(buttons),
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 1610284626 and DONATION_LINK:
            update.effective_message.reply_text(
                ...
                "[here](tg://openmessage?user_id=1060459378)",
                parse_mode=ParseMode.MARKDOWN,
            )
...
def main():
    ...
    test_handler = CommandHandler("testt", test)
