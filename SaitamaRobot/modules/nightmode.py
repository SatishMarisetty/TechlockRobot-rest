...
@tbot.on(events.NewMessage(pattern="/sleepmode (.*)"))
async def close_ws(event):

    if not event.is_group:
        await event.reply("Use this command on group")
        return
    ...
    if await is_admin(event, event.message.sender_id):
        if (
            ...
            if is_nightmode_indb(str(event.chat_id)):
                await event.reply("This Chat is Has Already Enabled Sleep Mode.")
                return
            ...
            if not is_nightmode_indb(str(event.chat_id)):
                await event.reply("This Chat is Has Not Enabled Sleep Mode. Use `/sleepmode on` to turn on")
                return
            ...
            await event.reply("I undestand `/sleepmode on` and `/sleepmode off` only")
    else:
...
scheduler.add_job(job_close, trigger="cron", hour=24, minute=00)
scheduler.start()
...
# Run everyday at 06
scheduler = AsyncIOScheduler(timezone="Asia/Kolkata")
scheduler.add_job(job_open, trigger="cron", hour=6, minute=00)
scheduler.start()
...
__help__ = """
<b> sleep mode </b>
Closes your group at 12.00 a.m. and open back at 6.00 a.m.(IST)
<i> Only available for asian countries (India Standard time)</i>

 `/sleepmode on`: Adds Group to SleepMode Chats
 `/sleepmode off`: Removes Group From SleepMode Chats
"""
