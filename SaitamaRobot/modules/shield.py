...
async def ws(event):
    ...
    if nude.is_nude("./nudes.jpg"):
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, TECHLOCK AI deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By TECHLOCK ` \n**#GROUP_GUARDIAN** "
            ...
            if event.chat_id == c["id"]:
                if better_profanity.profanity.contains_profanity(msg):
                    ...
                    if sender.username is None:
                        st = sender.first_name
                        hh = sender.id
                        final = f"[{st}](tg://user?id={hh}) Your message is detected as a slang word and your message has been deleted"
                    else:
                        final = f" Your message is detected as a slang word so your message has been deleted"
                    dev = await event.respond(final)
                    await asyncio.sleep(10)
                    await dev.delete()
        if event.photo:
